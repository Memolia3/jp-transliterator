...
const japanizer_1 = __importDefault(require("../../features/japanizer"));
describe("Japanizer", () => {
    let japanizer;
    beforeEach(() => {
        japanizer = new japanizer_1.default();
    });
    afterEach(() => {
        if (global.gc) {
            global.gc();
    ...
    describe("generatePatternArrayメソッド正常系", () => {
        it("4文字のローマ字パターンを正しく認識", () => {
            const japanizer = new japanizer_1.default();
            const spyFn = jest.spyOn(japanizer, "generatePatternArray");
    ...
    describe("generateAllCombinationsメソッド正常系", () => {
        it("文章を正しく返す", () => {
            const japanizer = new japanizer_1.default();
            const spyFn = jest.spyOn(japanizer, "generateAllCombinations");
            const pattern = [
                ["あ", "ア"],
                ["い", "イ"],
                ["う", "ウ"],
    ...
    describe("transformCombinationメソッド正常系", () => {
        it("正しく変換する", () => {
            const japanizer = new japanizer_1.default();
            const spyFn = jest.spyOn(japanizer, "transformCombination");
            const pattern = [[["あいうえお"], ["アイウエオ"]]];
            japanizer.transliterate("aiueo");
            expect(spyFn).toHaveBeenCalledWith(pattern);
    ...
    describe("並列処理テスト", () => {
        it("非同期で並行処理をテスト", async () => {
            ...
            const concurrentCalls = inputStrings.map((str) => new Promise((resolve) => {
                setTimeout(() => {
    ...
    describe("ストレステスト", () => {
        it("安定性を確認", async () => {
            ...
            const concurrentCalls = Array(numberOfCalls)
                ...
                resolve(japanizer.transliterate(randomInput));
            }));
